...
def _infer_shape(output_shape, dimensions):  # type: ignore
...
class Reshape():
  ...
  all dimensions between ``B`` and ``D``::
      ...
      >>> mod = Reshape(output_shape=(-1, D))
      >>> assert mod(x).shape == (B, H*W*C, D)

  You can change the number of preserved leading dimensions via
  ``preserve_dims``::

      >>> mod = Reshape(output_shape=(-1, D), preserve_dims=2)
      >>> assert mod(x).shape == (B, H, W*C, D)

      >>> mod = Reshape(output_shape=(-1, D), preserve_dims=3)
      >>> assert mod(x).shape == (B, H, W, C, D)

      >>> mod = Reshape(output_shape=(-1, D), preserve_dims=4)
...
class Flatten(Reshape):
  ...
  >>> x = jnp.ones([3, 2, 4])
  >>> negative_flat = Flatten(preserve_dims=-2)
  ...
  def __init__(self, preserve_dims: int = 1):
    """Init."""
    super().__init__(output_shape=(-1,), preserve_dims=preserve_dims)
