...
class CSVLogger(base.Logger):
  ...
  def __init__(
    self,
    directory_or_file: Union[str, TextIO] = '~/moss',
    label: str = '',
    time_delta: float = 0.,
    add_uid: bool = True,
    flush_every: int = 30,
  ...
  def _create_file(
    ...
    if isinstance(directory_or_file, str):
      ...
      return self._open(file_path, mode='a')  # type: ignore
  ...
  def write(self, data: base.LoggingData) -> None:
    ...
    if not self._writer:
      ...
      if not self._file.tell():
        self._writer.writeheader()  # type: ignore
    self._writer.writerow(data)  # type: ignore
